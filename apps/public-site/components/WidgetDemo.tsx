...
export function WidgetDemo({ widgetKey, apiUrl }: WidgetDemoProps) {
  if (!widgetKey) {
    return (
      <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-8 text-center text-slate-500">
        Демо-виджет будет доступен после настройки <code>DEMO_WIDGET_KEY</code>
      </div>
    );
  }

  const scriptSrc = `${siteConfig.widgetUrl}/embed.js`;
  ...
  return (
    <>
      ...
      <div className="rounded-2xl border border-slate-200 bg-gradient-to-br from-brand-50 to-white p-8">
        <p className="text-sm font-medium text-brand-700">Живое демо</p>
        <h3 className="mt-2 text-2xl font-bold text-slate-900">
          Попробуйте виджет прямо на этой странице
        </h3>
        <p className="mt-2 text-slate-600">
          Нажмите на кнопку чата в правом нижнем углу и задайте вопрос как
